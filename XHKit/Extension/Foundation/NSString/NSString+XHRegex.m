...
- (BOOL)xh_isMobileNumberClassification{
    ...
    if (([self xh_isValidateByRegex:CM])
        || ([self xh_isValidateByRegex:CU])
        || ([self xh_isValidateByRegex:CT])
        || ([self xh_isValidateByRegex:PHS]))
    {
        return YES;
    }
    else
...
- (BOOL)xh_isEmailAddress{
    ...
    return [self xh_isValidateByRegex:emailRegex];
}
...
- (BOOL)xh_simpleVerifyIdentityCardNum
{
    ...
    return [self xh_isValidateByRegex:regex2];
}
...
- (BOOL)xh_isCarNumber{
    ...
    return [self xh_isValidateByRegex:carRegex];
}

- (BOOL)xh_isMacAddress{
    NSString * macAddRegex = @"([A-Fa-f\\d]{2}:){5}[A-Fa-f\\d]{2}";
    return  [self xh_isValidateByRegex:macAddRegex];
}
...
- (BOOL)xh_isValidChinese;
{
    ...
    return [self xh_isValidateByRegex:chineseRegex];
}

- (BOOL)xh_isValidPostalcode {
    NSString *postalRegex = @"^[0-8]\\d{5}(?!\\d)$";
    return [self xh_isValidateByRegex:postalRegex];
}

- (BOOL)xh_isValidTaxNo
{
    NSString *taxNoRegex = @"[0-9]\\d{13}([0-9]|X)$";
    return [self xh_isValidateByRegex:taxNoRegex];
}
