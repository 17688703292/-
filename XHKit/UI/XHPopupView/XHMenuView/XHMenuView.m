...
@interface XHMenuView () <UITableViewDelegate, UITableViewDataSource>
...
- (void)configDefault {
    ...
    self.menuStyle = XHMenuDarkStyle;
}

- (void)configTable {
    [self addSubview:self.contentView];
    [self.mTable registerNib:[UINib nibWithNibName:@"XHMenuCell" bundle:nil]
      forCellReuseIdentifier:@"XHMenuCell"];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    XHMenuCell *cell = [tableView dequeueReusableCellWithIdentifier:@"XHMenuCell"
                                                            forIndexPath:indexPath];
    XHMenuModel *cellModel = self.menuArray[indexPath.row];
    cell.menuModel = cellModel;
    cell.menuStyle = self.menuStyle;
    cell.hideCellLine = self.hideCellLine;
    cell.menuCellLineColor = self.menuCellLineColor;
    cell.cellColor = self.menuBgColor;
    cell.menuCellFont = self.menuCellFont;
...
- (CGFloat)cacuateCellWidth {
    ...
    [self.menuArray enumerateObjectsUsingBlock:^(XHMenuModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)handleMenuModelArray:(NSArray *)array {
    ...
    [array enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        XHMenuModel *model  = [[XHMenuModel alloc] init];
...
+ (instancetype)pullMenuAnchorView:(UIView *)anchorView
    ...
    XHMenuView *menuView = [self pullMenuAnchorView:anchorView menuArray:nil];
...
+ (instancetype)pullMenuAnchorView:(UIView *)anchorView
                         menuArray:(NSArray<XHMenuModel *> *)menuArray {
    UIWindow * window=[[[UIApplication sharedApplication] delegate] window];
    XHMenuView *menuView = [[XHMenuView alloc] init];
...
+ (instancetype)pullMenuAnchorPoint:(CGPoint)anchorPoint
                         titleArray:(NSArray *)titleArray
                         imageArray:(NSArray *)imageArray {
    XHMenuView *menuView = [self pullMenuAnchorPoint:anchorPoint menuArray:nil];
    menuView.titleArray = titleArray;
    menuView.imageArray = imageArray;
    return menuView;
}
...
- (void)setMenuArray:(NSArray<XHMenuModel *> *)menuArray {
...
- (void)setMenuStyle:(XHMenuStyle)menuStyle {
    ...
    switch (menuStyle) {
        case XHMenuDarkStyle:
        {
            ...
            self.menuBgColor  = [UIColor darkGrayColor];
        }
            break;
...
- (void)setCoverBgColor:(UIColor *)coverBgColor {
    ...
    self.backgroundColor = coverBgColor;
}

- (void)setMenuBgColor:(UIColor *)menuBgColor {
    _menuBgColor = menuBgColor;
    self.mTable.backgroundColor = menuBgColor;
    [self refreshUI];
}
...
- (void)setMenuMaxHeight:(CGFloat)menuMaxHeight {
