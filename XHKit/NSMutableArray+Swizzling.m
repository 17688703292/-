...
+ (void)load {
  ...
  dispatch_once(&onceToken, ^{
    ...
    [objc_getClass("__NSArrayM") swizzleSelector:@selector(replaceObjectAtIndex:withObject:)
                              withSwizzledSelector:@selector(hdf_replaceObjectAtIndex:withObject:)];
...
- (void)hdf_replaceObjectAtIndex:(NSUInteger)index withObject:(id )obj {
    if (self.count <= 0) {
        NSLog(@"%s can't get any object from an empty array", __FUNCTION__);
        return;
    }
    ...
    [self hdf_replaceObjectAtIndex:index withObject:obj];
}
@end
