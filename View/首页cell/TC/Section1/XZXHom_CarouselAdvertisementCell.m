//
//  XZXHom_CarouselAdvertisementCell.m
//  BigMarket
//
...
@implementation XZXHom_CarouselAdvertisementCell
...
{
    if (!_scrollView) {
        _scrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 2, kScreenWidth, kScreenWidth*0.4)];
...
- (HQFlowView *)pageFlowView
{
    if (!_pageFlowView) {
        ...
        _pageFlowView = [[HQFlowView alloc] initWithFrame:CGRectMake(0, 2, kScreenWidth, kScreenWidth*0.4)];
...
{
    return CGSizeMake(kScreenWidth-2*30, self.scrollView.frame.size.height-2*3);
}
...
- (void)didSelectCell:(UIView *)subView withSubViewIndex:(NSInteger)subIndex
{
    if ([self.delegate respondsToSelector:@selector(DidSelectAdvertisementIndex:)]) {
        ...
        [self.delegate DidSelectAdvertisementIndex:subIndex];
    }
...
- (HQIndexBannerSubview *)flowView:(HQFlowView *)flowView cellForPageAtIndex:(NSInteger)index
{
    ...
    [bannerView.mainImageView sd_setImageWithURL:[NSURL URLWithString:self.advArray[index]] placeholderImage:[UIImage imageNamed:LoadPic]];
    //加载本地图片
